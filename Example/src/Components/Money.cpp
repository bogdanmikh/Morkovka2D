...
void Money::update(double deltaTime) {
    ...
    if (distanceTo(player) < 0.4f) {
        ...
        translate(player->getMinX() < getMinX()   ? -moveSpeed
                  : player->getMinX() > getMinX() ? moveSpeed
                                                  : 0,
            player->getMinY() < getMinY() && getMinY() - player->getMinY() > 0.1f   ? -moveSpeed
            : player->getMinY() > getMinY() && player->getMinY() - getMinY() > 0.1f ? moveSpeed
                                                                                    : 0,
            0.0);
    }
    draw();
